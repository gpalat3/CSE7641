...
def randomHillClimb(problem_size, weights, values, max_weight_pct):
    fitness = []
    times = []
    for length in problem_size:
        fitness_fn = mlrose_hiive.Knapsack(weights=weights[length], values=values[length], max_weight_pct=max_weight_pct)
    ...
    fitness_fn = mlrose_hiive.Knapsack(weights=weights[100], values=values[100], max_weight_pct=max_weight_pct)
    ...
    rhc = mlrose_hiive.RHCRunner(problem=problem, iteration_list=[1000], max_attempts=100, seed=99, restart_list=[0],
                                 experiment_name='RandomizedHillClimbing', output_directory='output/KnapSack/')
    rhc_stats, rhc_curves = rhc.run()
    return fitness, times, rhc_stats, rhc_curves

def simulatedAnnealing(problem_size, weights, values, max_weight_pct):
    fitness = []
    times = []
    for length in problem_size:
        fitness_fn = mlrose_hiive.Knapsack(weights=weights[length], values=values[length],
    ...
    sa = mlrose_hiive.SARunner(problem=problem, iteration_list=[1000], max_attempts=100, temperature_list=[250],
                               ...
                               output_directory='output/KnapSack/')
    sa_stats, sa_curves = sa.run()
    return fitness, times, sa_stats, sa_curves

def geneticAlg(problem_size, weights, values, max_weight_pct):
    fitness = []
    times = []
    for length in problem_size:
        fitness_fn = mlrose_hiive.Knapsack(weights=weights[length], values=values[length],
...
def mimic(problem_size, weights, values, max_weight_pct):
    fitness = []
    times = []
    for length in problem_size:
        fitness_fn = mlrose_hiive.Knapsack(weights=weights[length], values=values[length],
...
def plotIndCurves(x, y, title, xlabel, ylabel, label, savefile):
    plt.figure()
    plt.title(title)
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
...
if __name__ == '__main__':
    ...
    for i in problem_size:
        weights[i] = np.random.rand(i)
        values[i] = np.random.rand(i)
    max_weight_pct = 0.5
    fitness_rhc, times_rhc, stats_rhc, curves_rhc = randomHillClimb(problem_size, weights, values, max_weight_pct)
    fitness_sa, times_sa, stats_sa, curves_sa = simulatedAnnealing(problem_size, weights, values, max_weight_pct)
    fitness_ga, times_ga, stats_ga, curves_ga = geneticAlg(problem_size, weights, values, max_weight_pct)
    fitness_mimic, times_mimic, stats_mimic, curves_mimic = mimic(problem_size, weights, values, max_weight_pct)
    title, xlabel, ylabel = ['KnapSack - Fitness', 'Problem Size', 'Fitness']
    savefile = 'plots/KnapSack_Fitness_Problem_Size.png'
    plotCurves(problem_size, fitness_rhc, fitness_sa, fitness_ga, fitness_mimic, title, xlabel, ylabel, savefile)
    label = 'RHC'
    savefile = 'plots/KnapSack_Fitness_Problem_Size_' + label + '.png'
    plotIndCurves(problem_size, fitness_rhc, title, xlabel, ylabel, label, savefile)
    label = 'SA'
    savefile = 'plots/KnapSack_Fitness_Problem_Size_' + label + '.png'
    plotIndCurves(problem_size, fitness_sa, title, xlabel, ylabel, label, savefile)
    label = 'GA'
    savefile = 'plots/KnapSack_Fitness_Problem_Size_' + label + '.png'
    plotIndCurves(problem_size, fitness_ga, title, xlabel, ylabel, label, savefile)
    label = 'MIMIC'
    savefile = 'plots/KnapSack_Fitness_Problem_Size_' + label + '.png'
    plotIndCurves(problem_size, fitness_mimic, title, xlabel, ylabel, label, savefile)
    title, xlabel, ylabel = ['KnapSack - Time', 'Problem Size', 'Time']
    savefile = 'plots/KnapSack_Time_Problem_Size.png'
    plotCurves(problem_size, times_rhc, times_sa, times_ga, times_mimic, title, xlabel, ylabel, savefile)
    title, xlabel, ylabel = ['KnapSack - FEvals', 'Iteration', 'FEvals']
    savefile = 'plots/KnapSack_Iterations_FEvals.png'
